...
int main() {
	...
	ofstream inOut("gadgetData/receiptTemp.txt", ios::trunc); //masukkan data dalam file semata2 nak display gadget in receipt
	ifstream readDisplay("gadgetData/receiptTemp.txt"); //read input to display gadget yg dibeli at receipt
...
void writeIntoTemp(ofstream& tempGadget, ifstream& gadgetList, gadget gajet[], ofstream& inOut, ifstream& readDisplay, ofstream& TempAdmin, ifstream& Admin,double& sumAllBuyer,int& totalSales) {
	...
	TempAdmin << sumAllBuyer; // write data masuk TempAdmin punya file (dia ada 1 je data)
		...
		if (gajet[i].type == "P") { //PENDRIVE
			if (code == gajet[i].gadgetCode) {
				if (gajet[i].stock == 0) { 
					...
					cin >> quantity;
					if ((gajet[i].stock - quantity) < 0) { //bila user input more stocks than stock yang ada
						cout << "Please re-enter quantity because stock is not enough!" << endl << endl;
						goto startQuantity;
					}
					else if (cin.fail()) {  //salah input, x letak nombor
...
void displayAdminPage(double sumAllBuyer, string adminUser, gadget gajet[], ifstream& Admin,int& totalSales) { //admin page ( to display stocks, number of buyers and so on) 
	...
	if (adminSelect == 1) {
		...
		if (selectAddStock == 1) {
			...
			else { // kalau salah input gadget code
				cin.clear();
				cin.ignore(numeric_limits<streamsize>::max(), '\n');
	...
	else if (adminSelect == 2) { //Display no of item sold
...
void discountFunc(double& sum, string& coupon, double& diskaun) {
...
void receipt(double sum, ifstream& readDisplay, double& diskaun) { //receipt
